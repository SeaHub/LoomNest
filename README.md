...
│   └── assets/          # 正式宣传图和二维码
├── src/
│   ├── App.vue          # 页面渲染与加载异常提示，不是日常内容配置入口
...
- `src/App.vue` 在正式 JSON 加载失败时会显示异常提示和空列表，不会回退到虚构作品。
...
5. 页面没有出现作品数据加载失败提示，浏览器控制台没有配置加载错误。
...
6. 只修改本次接入需要的 `public/works.json` 记录和 `public/assets/` 素材。不要修改 `prototype/works.json`、`src/App.vue` 的加载逻辑、`dist/`、`node_modules/` 或无关项目。
7. 保留现有数组、记录和顺序，除非用户明确指定插入位置或要求调整顺序。避免无关格式化和重构。
8. 修改后运行 JSON/ID 检查、`npm test` 和 `npm run build`，并进行页面检查。不得把数据加载失败视为成功。
...
| 现象 | 检查与处理 |
| --- | --- |
| 页面提示“作品数据加载失败” | 检查 `public/works.json` 是否为合法、非空的 JSON 数组；检查开发服务器是否能请求到正确的 `works.json` 路径。修复前不能认定接入成功。 |
